...
def RunML(kmr,Root,dtp,MLname,TrnWholeD,TrD,TrL,TsD,TsL,n):
...
def main():
    ...
    for dtp in dtpL:
        ...
        MLAlg=['SVMi']#['RF','KNN','SVM']

        for ml in tqdm(MLAlg):
            begin_time=datetime.now()
            # print(f'{ml} processing at:{begin_time}')
            print('--------------------------------')
            if TrnWholeD==True:
                n=0
                RunML(kmr,Root,dtp,ml,TrnWholeD,TrD,TrL,TsD,TsL,n)
