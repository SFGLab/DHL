...
def loadDataSet(obj,Root,dtp,tsC):
    '''
...
def RunMLfld(kmr,Root,dtp,MLname,TrnWholeD,DtX,DtY,TsD,TsL,n):
    ...
    for ind in tqdm(range(n)):
        ...
        CLSF=load(f"{modelDir}/{MLname}_F{ind}.joblib")
        print(f"{dtp} Fold-{ind} model loaded")
...
def loadModel(n):
    ...
    for ind in tqdm(range(n)):
        ModlDic
def RunMLpred(kmr,Root,dtp,MLname,TrnWholeD,TsD,TsL,n):
    CLSF=classifiers[MLname]
    sln="Classifier,Fold,AUC,auPRC,Accu,Pre,Recall,F1,MCC\n"
    rSL="Classifier,Fold,AUC,AUPRC,Accu,Pre,Recall,F1,MCC\n"
    hSL="Classifier,Fold,AUC,AUPRC,Accu,Pre,Recall,F1,MCC\n"
    ...
    if TrnWholeD:
        ...
        CLSF=load(f"{modelDir}/{MLname}_WholeD.joblib")
...
def main():
    ...
    parser.add_argument('-w', "--whld",type=int,help=" :1 / 0",required =True)    
    ...
    print(f"kmer length:: {kmr},\nTrnWholeD:: {TrnWholeD},\nFold:: {n}")#, \nTrainMode:{ptmode}\t PredMode:{not(ptmode)}")
    ...
    #dln={'RNAPOL2':{'tr':1000,'ts':1000},'CTCF':{'tr':600,'ts':600}}
    dln={'RNAPOL2':{'tr':300000,'ts':15000},'CTCF':{'tr':500000,'ts':20000}}
    for dtp in dtpL:
        trC=dln[dtp]['tr'];tsC=dln[dtp]['ts']
        TsD,TsL=loadDataSet(obj,Root,dtp,tsC)
        # print(TrD.shape,TrL.shape)

        MLAlg=['RF','KNN']#,'SVM']'SVMi']#,
        ...
        if TrnWholeD==0:
            for ml in tqdm(MLAlg):
                ...
                print('--------------------------------')
                RunMLpred(kmr,Root,dtp,ml,TrnWholeD,TsD,TsL,n)
